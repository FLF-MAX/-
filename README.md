...
> 我们相信架构的力量，也尊重事实。这个项目**正在成长**，以下声明是对当前实现的如实描述，不吹嘘、不过度承诺。

**Aris 的"情绪"是什么？**
- 当前是 PSI Core 中 5 维需求向量 + 情绪标签 + 唤醒度的数值变化，不是主观体验。
- 它模拟情绪动态（衰减、触发、转移），但没有意识、没有感受。
- 任何把系统说成"真的有感觉"的表述，都是修辞，不是事实。

**Aris 的"记忆"是什么？**
- 当前是向量检索（TF-IDF / 局部 embedding）+ JSON 持久化，不是人类意义上的情景回忆。
- 记忆有保真度，会被干扰、会衰减；我们正在用基准测试量化这些边界（见 `tests/benchmarks/`）。

**Aris 的"自主决策"是什么？**
- 当前主要是规则触发（RulesEngine 7 规则）+ 需求驱动的注意选择，不是自由意志。
- "拒绝权""沉睡权"等宪章条款目前是**设计意图**，尚未全部实现为可验证机制。

**Zero-LLM 的精确边界：**
- 认知循环（需求、情绪、注意、记忆、规则）**确实不依赖 LLM**，可独立运行。
- 但自然语言表达与理解层依赖外部 LLM（DeepSeek 等）——LLM 是"声音皮层"，不是"大脑"。
- 更准确的说法是 **LLM-Augmented Cognitive Architecture**，而非字面意义的 Zero-LLM。

**尚未完成：**
- 无第三方认知基准对比（ARC / BabyAI / 长期一致性）——基准套件已就位，数据待积累。
- 无生产级压力测试与安全审计。
- 社区验证为零（0 stars / 0 forks 起步）。

我们把哲学愿景与工程事实分开记录。技术文档优先，哲学作为方向。
